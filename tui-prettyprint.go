...
func BoardString(b *chess.Board, sel []chess.Position, cursor chess.Position) (result string) {
	...
	result += printRank(b.Grid[6], true, 7, sel, cursor, fmt.Sprintf("  advantage for white: %v", GetMaterialStats(b).GetAdvantage("white")))
	...
	result += printRank(b.Grid[1], false, 2, sel, cursor, fmt.Sprintf("   bot evaluation: %v", botEvaln))
